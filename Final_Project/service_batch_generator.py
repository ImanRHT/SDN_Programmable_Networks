...
class ServiceBatchGenerator(object):
    ...
    def __init__(self, batch_size, min_service_length, max_service_length, vocab_size):
        ...
        self.state = np.zeros(
            (self.batch_size, self.max_service_length),  dtype='int32')
    ...
    def getNewState(self):
        ...
        for batch in range(self.batch_size):
            self.service_length[batch] = np.random.randint(
                self.min_service_length, self.max_service_length+1, dtype='int32')
...
if __name__ == "__main__":
    ...
    env = ServiceBatchGenerator(
        batch_size, min_service_length, max_service_length, vocab_size)
    env.getNewState()
